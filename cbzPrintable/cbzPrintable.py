...
def sort_func(input_file):
    if args.file_pattern is None:
        ...
        return float(input_file.split(args.file_pattern.split('*')[0])[1].split(args.file_pattern.split('*')[1])[0])
...
def main():
    try:
        ...
        os.system(sys.executable +
                  ' -m pip install -r CbXManager\\requirements.txt')
            ...
            for images in to_pack:
                if len(images) == 1:
                    ...
                    img_data = requests.get(
                        "https://upload.wikimedia.org/wikipedia/commons/thumb/5/50/Black_colour.jpg/1200px-Black_colour.jpg").content
